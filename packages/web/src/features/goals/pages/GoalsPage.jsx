...
export function GoalProgressBar({ current, target }) {
  ...
  return (
    <div className="h-2.5 w-full overflow-hidden rounded-full bg-white/[0.07]">
      <div
        className="h-full rounded-full bg-gradient-to-r from-primary to-secondary shadow-[0_2px_8px_rgba(0,0,0,0.3)] transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)]"
        style={{ width: `${pct}%` }}
      />
    </div>
  )
      ...
      {!done && (
        ...
        >
          <input
            ...
            className="min-w-0 flex-1 rounded-[10px] border border-slate-200/15 bg-bg-primary/60 px-3 py-2 text-sm text-text-primary placeholder:text-text-subtle outline-none transition-all focus:border-primary/50 focus:shadow-[0_0_0_3px_rgba(139,92,246,0.12)]"
          />
