...
export default function ProgressRing({ percent = 0, size = 'md', label, glow = false, from = '#8b5cf6', to = '#a78bfa' }) {
  ...
  return (
    <div
      ...
      style={{ width: d, height: d, filter: glow ? `drop-shadow(0 0 ${Math.round(d / 12)}px ${from}33)` : undefined }}
    >
      <svg width={d} height={d} className="-rotate-90">
        ...
        <circle cx={d / 2} cy={d / 2} r={r} fill="transparent" stroke="rgba(255,255,255,0.06)" strokeWidth={stroke} />
        <circle
          cx={d / 2}
          cy={d / 2}
          r={r}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className={`font-display font-bold tabular-nums text-text-primary ${text}`}>{Math.round(clamped)}%</span>
