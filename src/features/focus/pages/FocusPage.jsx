...
function TimerRing({ progress, isBreak, paused, running, children }) {
  ...
  const c = 2 * Math.PI * r
  const from = isBreak ? '#00e29a' : '#ff7ad9'
  const to = isBreak ? '#22d3ee' : '#a855f7'
  return (
    <motion.div
...
export default function FocusPage() {
  ...
  return (
    <div className="hero-spot relative mx-auto flex max-w-[600px] flex-col items-center" style={{ '--accent': '#ff7ad9' }}>
      ...
      <AnimatePresence mode="wait">
        <motion.span
          ...
          className={`mb-8 rounded-full border px-6 py-2 font-display text-xs font-bold uppercase tracking-[0.1em] ${
            mode === 'work'
              ? 'border-accent/35 bg-accent/[0.12] text-[#ffb3e6] shadow-[0_0_28px_-6px_rgba(255,122,217,0.5)]'
              : 'border-success/35 bg-success/[0.12] text-[#7df0c8] shadow-[0_0_28px_-6px_rgba(0,226,154,0.5)]'
          }`}
        >
      ...
      <div className="card-glass mt-12 w-full p-7">
        <div className="mb-6">
          <div className="mb-2.5 flex justify-between text-sm">
            ...
            <span className="font-brand-mono text-[#ffb3e6]">{workMin} min</span>
          </div>
          <input
            ...
            style={{ accentColor: '#ff7ad9' }}
          />
        </div>
        <div>
          <div className="mb-2.5 flex justify-between text-sm">
            <span className="font-display font-semibold">Break duration</span>
            <span className="font-brand-mono text-[#7df0c8]">{breakMin} min</span>
          </div>
          <input
